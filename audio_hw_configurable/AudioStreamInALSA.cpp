...
{
    AUDIOCOMMS_ASSERT(effect!= NULL && isRouteAvailable(), "effect handle is NULL or not routed");
...
status_t AudioStreamInALSA::getAudioEffectUuidFromHandle(effect_handle_t effect,
                                                         effect_uuid_t *uuid)
{
    AUDIOCOMMS_ASSERT(effect != NULL && uuid != NULL, "effect handle or UUID is NULL");
