...
AudioResamplerALSA::AudioResamplerALSA(int32_t outSampleRate, int32_t inSampleRate,
    ...
    mInSampleRate(inSampleRate), mChannelCount(channelCount), mContext(0),
    mFloatInp(0), mFloatOut(0), mResult(0), mResultCount(0), mMaxFrameCnt(0)
{
...
void AudioResamplerALSA::convert_short_2_float(int16_t *inp, float * out, unsigned int sz)
{
    int i;
    for (i = 0; i < sz; i++) {
        *out++ = (float) *inp++;
...
void AudioResamplerALSA::convert_float_2_short(float *inp, int16_t * out, size_t sz)
{
    int i;
    for (i = 0; i < sz; i++) {
        if (*inp > SHRT_MAX) {
